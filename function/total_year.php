...
$query = "SELECT SUM(leaves.percent)  AS percent, DATE_FORMAT(leaves.date_time, '%Y') AS date_time FROM leaves
...
$query = "SELECT SUM(leaves.percent) AS percent, DATE_FORMAT(leaves.date_time, '%Y') AS date_time FROM leaves
 LEFT JOIN student ON student.id_std = leaves.id_std
     WHERE leaves.id_std= '$my_id'
     GROUP BY DATE_FORMAT(leaves.date_time, '%Y%') ";
